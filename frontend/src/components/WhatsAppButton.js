...
export const WhatsAppButton = () => {
  ...
  return (
    ...
    >
      <span className="absolute inset-0 rounded-full bg-[#9B1B1B] opacity-40 animate-ping" style={{ animationDuration: "2.5s" }} />
      <span className="relative flex items-center gap-2 rounded-full bg-[color:var(--lux-obsidian)] border border-[color:var(--border-gold)] pl-3.5 pr-4 py-3.5 shadow-[0_10px_30px_rgba(0,0,0,0.55)] transition-colors group-hover:bg-[color:var(--lux-charcoal)]">
        <span className="absolute -top-0.5 -right-0.5 h-3 w-3 rounded-full bg-[#C0392B] ring-2 ring-[color:var(--lux-obsidian)]" />
        <svg viewBox="0 0 32 32" className="h-6 w-6 fill-[color:var(--lux-gold)]" aria-hidden="true">
        ...
        <span className="hidden sm:block text-[color:var(--lux-gold)] text-sm font-semibold pr-1">WhatsApp</span>
      </span>
    </a>
  );
};
